## --- 1) Folds: contiguous time blocks ---
...
## --- 2) Predictive log-likelihood for a given theta on pre-made folds ---
## theta = c(space_length_scale, time_periodic_scale, time_long_term_scale)
...
## --- 3) Wrapper: subsample sites, build folds, then optimise with L-BFGS-B ---
## bounds must be positive and within plausible ranges for your data
...
## --- 4) Example usage ---
#pick sensible bounds for your context (units must match your kernels!)
...
space_pd <- data.frame(
  ...
  mutate(
    k = rbf_kernel(distance, theta = res$best_theta[1]),
    group = "True"
  )
...
time_pd <- data.frame(
  month = seq(0, (nt), length.out = nt * 10)
) |>
  mutate(
    k = periodic_kernel(x = month, alpha = res$best_theta[2], period = 12) *
      rbf_kernel(x = month, theta = res$best_theta[3]),
    group = "True"
  )

time_plot <- ggplot(time_pd, aes(x = month, y = k, colour = group)) +
  geom_line() +
  theme_bw()

space_plot + time_plot
