...
#' Case counts:
#'  y_st ~ nbinom(λ_st, size)
#'  # As size -> Inf, nbinom -> pois
#' Cases mean:
#'  λ_st = exp(z_st)
...
estimate <- fit(obs_data, coordinates, nt, period, n_sites = 10, mask_prop = 0.2,
                par0 = c(1, 5, 100, 40),  lower = c(1e-4, 0.8, 52 * 1.5, 0.5), upper = c(2, 10, 500, 1000)
)
...
interpolation_nb_plot <- ggplot() +
  ...
  ggtitle("Interpolation: Negative binomial (simulated data)")
...
ggsave("plots/interpolation_negative_binomial_plot.pdf", interpolation_nb_plot, height = 8, width = 15)
